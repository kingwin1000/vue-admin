      ...
      <el-form v-if="index === null" ref="dataForm" :key="1" :model="formAddData" :rules="addRules">
        ...
        <el-form-item label="角色" label-width="80px">
          <el-checkbox-group v-model="formAddData.role_ids">
            <el-checkbox v-for="item in rolesList" :key="item.id" :disabled="item.status == '0'" :label="item.id">{{ item.name }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
      </el-form>

      <el-form v-else ref="dataFormEdit" :key="2" :model="formEditData" :rules="editRules">
        <el-form-item label="用户名" prop="username" label-width="80px">
          <el-input v-model="formEditData.username" disabled placeholder="请输入用户名" auto-complete="off" />
        </el-form-item>
...
export default {
    ...
    rolesFilter(roleArr, rolesList) {
      if (rolesList && rolesList.length > 0) {
        ...
        roleArr.forEach(item => {
          const _role = rolesList.find(role => role.id == item)
          roleStr += _role.name + '/'
        })
  ...
  data() {
    ...
    const validateNumLetterOrEmpty = (rule, value, callback) => {
    ...
    const validatePass22 = (rule, value, callback) => {
