...
<div class="app-container">
  ...
  <div class="course-class">
          ...
          <el-tree :data="data" node-key="id" v-loading="loading" default-expand-all :expand-on-click-node="false" :props="defaultProps">
            <span style="flex: 1; display: flex; align-items: center; justify-content: space-between; padding-right: 8px;"  slot-scope="{ node, data }">
                ...
                <span class="operation-col">
                  <input class="order-list" @blur ="() => setOrder(data)" v-model="data.list_order">
    ...
    <el-dialog title="添加一级分类" :visible.sync="formVisible" :before-close="hideForm" :close-on-click-modal="false" width="50%" top="5vh">
      <el-form  :model="formData" :key="1" :rules="formRules" ref="dataForm" label-width="80px" style="padding-right:25px;">
        ...
        <el-form-item label="分类名称" prop="name">
          <el-input v-model="formData.name" placeholder="分类名称" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="所属路径" prop="path">
          <el-input v-model="formData.path" placeholder="所属路径" auto-complete="off"></el-input>
        </el-form-item>              
      </el-form>
    ...
    <el-dialog title="添加子分类" :visible.sync="subCategoryFormVisible" :before-close="hideSubCategoryForm" :close-on-click-modal="false" width="50%" top="5vh">
      <el-form  :model="subCategoryFormData" :key="2" :rules="formRules" ref="dataForm" label-width="80px" style="padding-right:25px;">
        ...
        <el-form-item label="所属分类" prop="parent_name">
          <el-input v-model="subCategoryFormData.parent_name" disabled placeholder="父分类名称" auto-complete="off"></el-input>
        </el-form-item>              
        <el-form-item label="分类名称" prop="name">
          <el-input v-model="subCategoryFormData.name" placeholder="分类名称" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="所属路径" prop="path">
          <el-input v-model="subCategoryFormData.path" placeholder="所属路径" auto-complete="off"></el-input>
        </el-form-item>              
      </el-form>
      <div slot="footer" class="dialog-footer">
          <el-button @click.native="hideSubCategoryForm">取消</el-button>
          <el-button type="primary" @click.native="subCategoryFormSubmit()" :loading="formLoading">提交</el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog title="编辑" :visible.sync="curCategoryFormVisible" :before-close="hideCurCategoryForm" :close-on-click-modal="false" width="50%" top="5vh">
      <el-form  :model="curCategoryFormData" :key="3"  label-width="80px" style="padding-right:25px;">
        ...
        <el-form-item label="分类名称" prop="name">
          <el-input v-model="curCategoryFormData.name" placeholder="分类名称" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="所属路径" prop="path">
          <el-input v-model="curCategoryFormData.path" placeholder="所属路径" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="选中路径" prop="active_path">
          <el-input v-model="curCategoryFormData.active_path" placeholder="所选中路径/各父元素path" auto-complete="off"></el-input>
        </el-form-item>         
      ...
      <div slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click.native="curCategoryFormSubmit()" :loading="formLoading">提交</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    async getList() {
      this.loading = true; 
      let res = await request.get(config.listUrl); 
      this.loading = false;
      if(res.code == '20000'){
    ...
    formSubmit(){
      this.$refs["dataForm"].validate(async valid => {
        if (valid) {
          ...
          var res = await request.post(config.addMenuSave,this.formData); 
    ...
    remove(data) {
      ...
      .then(async () => {
         var res = await request.post(config.delMenuUrl,{'id':data.id});
    ...
    async edit(data){   
      let res = await request.get(config.detUrl,{params:{id:data.id}});
      if(res.code == '20000'){
        this.curCategoryFormVisible = true;
        let _info = res.data && res.data.info ? res.data.info : '';
        if(_info){
...
.course-class>>>  .classify >>> .operation-col >>> .el-tree-node__content {
      height: 32px;
      margin: 0; 
