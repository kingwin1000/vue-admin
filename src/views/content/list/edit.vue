  ...
  methods: {
      ...
      if(categories.code == '20000'){
        this.categories = categories.data;
			...
			let tags = await request.get(config.contentTags);
      if(tags.code == '20000'){
        this.tags = categories.data;
      }
			let content = await request.get(config.content,{ params:{id:this.id} });
			if(content.code == '20000'){
				var _content = content.data[0]
				this.formData = {
					title:_content.title,
					titleImg:_content.titleImg,
					introduct:_content.introduct,
					categories:_content.categories,
					tags:_content.tags,
					content:_content.content,
					status:0,
				}
				this.imageUrl = _content.titleImg ? config.host+_content.titleImg : '';
		...
		handleSave(){
			this.$refs["dataForm"].validate(async valid => {
				if(valid){
					...
					this.formData.updateDate = new Date().getTime();
					var res = await request.put(config.content+'/'+this.id,this.formData);	
					if(res.code == '20000'){
